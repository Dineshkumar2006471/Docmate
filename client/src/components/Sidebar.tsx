    ...
    const sidebarContent = (
        <div className="flex flex-col h-full">
            ...
            <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
                ...
                ${isActive ? 'bg-primary-500/10 text-primary-400' : 'text-slate-400 hover:text-slate-100 hover:bg-white/5'}
              `}>
                                {isActive && (
                                    <motion.div
                                        layoutId="activeTab"
                                        className="absolute inset-0 bg-primary-500/10 border-l-2 border-primary-400"
                                        initial={false}
                                        transition={{ type: "spring", stiffness: 300, damping: 30 }}
                                    />
                                )}
                                <item.icon className={`w-5 h-5 relative z-10 ${isActive ? 'text-primary-400' : 'group-hover:text-primary-400 transition-colors'}`} />
                                {(!isCollapsed || mobileOpen) && (
        ...
        <>
            ...
            <motion.div
                className={`
                    ...
                    bg-surface-highlight/95 md:bg-surface-highlight/30 backdrop-blur-xl border-r border-slate-800 
